...
def everything_for_your_cat(cats_data):
    """
    Функция позволяеет отобразать в одной строке имя владельца и всех его животных.
    :param cats_data: Информация о животных и их владельцев.
    :return: Список животных по каждому владельцу.
    """
    result = {}
    our_str = ''
    for cat_info in cats_data:
        temp = cat_info[0] + ', ' + str(cat_info[1])
        result.setdefault(cat_info[2:], []).append(temp)
    for c, h in result.items():
        owner = (' '.join(c) + ': ', '; '.join(h) + '\n')
        res = ''.join(owner)
        our_str += str(res)
    return our_str
